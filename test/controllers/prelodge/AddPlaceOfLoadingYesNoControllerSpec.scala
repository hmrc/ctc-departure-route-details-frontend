...
import navigation.{LoadingAndUnloadingNavigatorProvider, RouteDetailsNavigatorProvider}
...
class AddPlaceOfLoadingYesNoControllerSpec extends SpecBase with AppWithDefaultMockFixtures with MockitoSugar {
  ...
  "AddPlaceOfLoadingYesNoControllerSpec" - {

    "must return OK and the correct view for a GET" in {

      setExistingUserAnswers(emptyUserAnswers)

      val request = FakeRequest(GET, addPlaceOfLoadingYesNoRoute)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request = FakeRequest(POST, addPlaceOfLoadingYesNoRoute)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request   = FakeRequest(POST, addPlaceOfLoadingYesNoRoute).withFormUrlEncodedBody(("value", ""))
